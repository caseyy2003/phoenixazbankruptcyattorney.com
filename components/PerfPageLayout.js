...
/* Build the Next optimizer URL (use allowed widths like 640/828/1080/1200/1920) */
...
export default function PerfPageLayout({
  ...
  hero = {
    ...
    // NEW: sane defaults to avoid oversized hero requests
    priority: false,
  ...
  // Background image mode (keeps text/CTA as likely LCP). Use allowed widths 640 (1x) & 828 (2x).
  ...
  // Use the same sizing logic everywhere (plain img + next/image)
  const resolvedSizes = hero.sizes || "(max-width: 768px) 100vw, 1200px";
  const resolvedQuality = typeof hero.quality === "number" ? hero.quality : 60;
  const resolvedPriority = !!hero.priority;
      ...
      <Footer
        ...
        content={
          <div style={{ textAlign: "center", padding: "1rem", fontSize: "0.9rem", color: "#555" }}>
            <div>© {new Date().getFullYear()} Yontz Law, PLLC.</div>
            ...
            <div style={{ marginTop: "0.5rem", fontSize: "0.85rem", color: "#777" }}>
              The content on this site is informational only and not legal advice. Always consult with a qualified
              Arizona bankruptcy attorney directly for legal guidance Absent a written fee agreement, no attorney-client
